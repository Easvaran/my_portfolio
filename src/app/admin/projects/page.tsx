...
export default function AdminProjects() {
  ...
  const fetchSectionData = async () => {
    try {
      const res = await fetch('/api/content?section=projects');
      if (res.ok) {
      ...
      console.error('Failed to fetch projects section data:', err);
    }
  };
            ...
            <div className="mb-12 p-8 rounded-[40px] bg-card/50 border border-white/10 shadow-2xl backdrop-blur-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    type="text" 
                    value={sectionData.githubLink} 
                    onChange={e => setSectionData({...sectionData, githubLink: e.target.value})} 
                    className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 focus:border-primary outline-none text-white font-bold" 
              ...
              <button
                ...
                className="flex items-center gap-3 px-8 py-4 bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-widest rounded-xl transition-all border border-white/10"
              >
                {savingSection ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                Update Section Text
              </button>
            </div>
